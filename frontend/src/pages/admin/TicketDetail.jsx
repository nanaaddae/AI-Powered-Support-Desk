...
export default function AdminTicketDetail() {
  ...
  useEffect(() => {
    ...
    ]).then(([ticketRes, commentsRes, agentsRes]) => {
    ...
    <Layout>
      <div className="p-8 max-w-3xl mx-auto">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 mb-5 space-y-4">
          <h3 className="text-sm font-medium text-white">Ticket controls</h3>
          ...
          {/* Assign agent */}
          <div>
            <p className="text-xs text-gray-500 mb-2">Assigned agent</p>
            <select
              value={ticket.assignedAgentId ?? ''}
              onChange={(e) => handleAssign(e.target.value)}
              className="bg-gray-800 border border-gray-700 text-gray-300 text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-indigo-500"
            >
          ...
          {classification && (
            <div className="bg-gray-800 rounded-lg p-4 mb-3">
              ...
              <div className="flex gap-3 mb-2">
                <span className="text-xs bg-indigo-950 text-indigo-400 border border-indigo-800 px-2 py-0.5 rounded-full">{classification.category}</span>
                <span className={`text-xs px-2 py-0.5 rounded-full border ${priorityBadgeColors[classification.priority]}`}>{classification.priority}</span>
              </div>
              ...
              {suggestions.map((option, i) => (
                <div key={i} className="bg-gray-800 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button onClick={() => copyToComment(option.response)} className="text-xs text-gray-500 hover:text-white transition">Use this</button>
                  </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-5 mb-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={handleKbSuggestions} disabled={loadingKb}
          ...
          {kbSuggestions.length === 0 ? (
            <p className="text-sm text-gray-500">Click "Find relevant articles" to surface related knowledge base articles.</p>
          ) : (
              ...
              {kbSuggestions.map((doc) => (
                <div key={doc.id} className="bg-gray-800 rounded-lg p-3">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <button onClick={() => setExpandedKb(expandedKb === doc.id ? null : doc.id)}
