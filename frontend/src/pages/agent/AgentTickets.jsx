    ...
    <Layout>
      <div className="p-8 max-w-5xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-400 text-sm mt-1">{tickets.length} ticket{tickets.length !== 1 ? 's' : ''} assigned</p>
        </div>
                ...
                {tickets.map((ticket) => (
                  <tr key={ticket.id} className="hover:bg-gray-800 transition">
                    <td className="px-5 py-3.5">
                      <Link to={`/agent/tickets/${ticket.id}`} className="text-white hover:text-indigo-400 transition">
                        {ticket.title}
                      </Link>
                    </td>
